...
import { AgregarRegistro } from './ArrayObjects.js';
import { RetornarSoluciones } from './ArrayObjects.js';
...
export function MostrarSoluciones() {
    ...
    ListadoSoluciones2.map((x) => {
      ...
      <div class="card border-secondary mb-3" header bg-secondary>
        ...
        <h5 class="card-title">${x["idMensajeError"]} "ID del mensaje de error" ${x["nombreError"]} "Nombre del error"</h5> 
        </div>
        <div class="card-body">
            <h5 class="card-title">Codigo del error: ${x}</h5>
            <p class="card-text">Nivel de complejidad: ${x}</p>
            <h5 class="card-title">Descripcion del error: ${x}</h5>
            <p class="card-text">Reportado por: ${x}</p>
        </div>
        <img src="..." class="card-img-bottom" alt="${x}">
      </div>
